...
S.exp_name = 'SAL'; %change this to flexibly redirect the script to different studies in subdirectories
...
S.trainTask = 'IntvsPros';%Circmaze - 'goals' or 'plan'

% ~~~ WHAT is the ROI we're analyzing
S.roi_name = 'bilatAll'; %S.roi_name = 'HVisCtx_1.nii'; %S.roi_name = 'NativeGM_BOLDres.nii';
...
S.testTask = 'IntvsPros';%Circamze - 'goals' or 'plan'
...
S.trainonsfname = '_Model_IntvsPros';
S.testonsfname = '_Model_IntvsPros';
...
par.preproc_lvl = 'swua'; % 'a' for slice-time-only, 'u' for realigned-only, 'ua' for realign+unwarped, 'swua' for smoothed, normalized, and... you get the picture. Modify as needed if you changed SPM's prefix append defaults
par.boldnames = [par.preproc_lvl 'run']; %name of image files with preprocessing level prefix
par.runnames = 'run*'; %what are your run/scan session folders called? If none or only 1 run, set to '' or whatever may be appropriate.
...
par.ref_funcimage = [par.boldnames '01_00010' par.imageextension];
...
% live
S.sbasepath = 'C:\Users\5328r\Documents\Homewrk\Research\ProcessingFolder\mvpa_sample_data\';

% ~~~ WHAT IS THE NAME *of your model folder's directory*?
modDir = strcat('results-',subj_id);
S.modfold = modDir{1};
S.modfold_singlebetas = 'results-01/simpleLSA'; % if your single trial (or person, ...
...
%establishment || Smaller penalty = stronger penalty because in this
% algorithm, it's in the denominator. This is somewhat odd compared to other
% algorithms 
...
elseif strcmp(S.trainTask,'IntvsPros')
    S.onsetsTrainDir = [S.mvpa_dir];%directory containing onsets.mat or betas_idx.mat file to be loaded in
    S.condsTrain = {{'intact'}  {'pros'}} ;%corresponds to the names in the onsets.mat or betas_idx.mat files. This is used to select what is being compared with what.
    S.TrainRuns = par.scansSelect.(par.task).loc;%pull up indexing, defined above, for RUNS corresponding to task of interest (i.e. if runs 2,4,6 correspond to task 1)
    if strcmp(S.inputformat, 'raw')
        S.filenames_train = raw_filenames;%
    elseif strcmp(S.inputformat, 'betas')
...
elseif strcmp(S.testTask,'IntvsPros')
    S.onsetsTestDir =[S.mvpa_dir];%directory containing onsets.mat or betas_idx.mat file to be loaded in
    S.condsTest = {{'intact'} {'pros'}};
    S.nwayclass = num2str(numel(S.condsTest));%stores the number classification dimensions just for reference (i.e. is this a 5-way or a 2-way/binary classification?)
    S.TestRuns = par.scansSelect.(par.task).loc;
    if strcmp(S.inputformat, 'raw')
        S.filenames_test = raw_filenames;%
...
if par.ImgDims == 3
    if ~strcmp(par.runnames,'') % add contingency for when all the raw filenames are just dumped in your main funcdir (i.e., there are no runfolds)
        for idx = 1:length(raw_filenames)
            ...
            runidxnum = str2double(raw_filenames{idx,1}(runref_indices(1)+4:runref_indices(1)+5));
